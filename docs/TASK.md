...
### P14-T10. Admin Content Read View
- 의존성: P14-T9B(Admin Visual Foundation - `.admin-page` surface/page header/button/link 스타일 재사용)
- 산출물: `AdminBoardViewController`/`AdminProgramViewController`(변경, `GET /{id:\d+}` 추가), `admin/board/detail.html`/`admin/program/detail.html`(신규), `admin/board/list.html`/`admin/program/list.html`(변경, 제목 링크), `static/css/admin/admin.css`(변경), `AdminBoardViewControllerTest`/`AdminProgramViewControllerTest`/`frontend-tests/visual-regression.spec.js`(변경), `docs/ARCHITECTURE.md`/`docs/API.md`/본 문서
- 작업 내용: 관리자가 게시글/프로그램 내용을 확인하려면 수정 폼을 열어야 했던 문제(목록 제목이 `textContent` 일반 텍스트이고 행 진입점이 "수정"뿐)를 해결한다. **읽기 전용 관리자 상세 View** `GET /admin/boards/{id}`, `GET /admin/programs/{id}`를 추가하고 목록 제목을 상세 링크로 바꾼다(링크 텍스트는 계속 `textContent`, 기존 수정/공개 전환/모집 상태 전환/삭제와 열 구조 무변경, 별도 "보기" 열 없음). 상세는 기존 `BoardService.getAdminById`/`ProgramService.getAdminById`(공개 여부 무관)와 `BoardResponse`/`ProgramResponse`를 그대로 재사용하고, 본문은 Public 상세와 동일하게 `ContentLinkRenderer.externalLinksOpenInNewTab` → model `renderedContent` → `th:utext`로만 출력한다(ARCHITECTURE.md "Admin 화면(View) / API 컨트롤러 명명 규칙"의 읽기 전용 상세 예외). 표시 항목 - Board: 제목, 게시판 유형(REVIEW는 `programType` enum 값 병기), 공개 상태, 등록일, 수정일, 대표 이미지(있을 때), 본문, 첨부파일(있을 때). Program: 제목, 프로그램 유형, 모집 상태, 공개 상태, 신청 URL(있을 때 외부 링크), 등록일, 수정일, 대표 이미지, 본문, 첨부파일. 상단 action - [목록으로](`/admin/boards`·`/admin/programs` 고정), [수정](`/{id}/edit`), [공개 페이지 보기](`isPublic`일 때만, 새 탭, Program은 CLOSED여도 공개면 표시). 본문 CKEditor 스타일은 `home.css`를 import/분할하지 않고(P14 공통 계약 A-13) 관리자 상세에 필요한 규칙만 `.admin-content-detail .ckeditor-content` 범위로 admin.css에 복사한다.
- 변경 금지: Entity/Repository/Service/Response DTO/`SecurityConfig`/`AdminNavigationControllerAdvice`/`GlobalExceptionHandler`/`HtmlSanitizer`/`ContentLinkRenderer`/Public Controller·template/`home.css`/Migration/Dashboard/Page/Popup/Banner/HomePinned/Menu/Theme/File/CKEditor 기능/전역 cache 정책/신규 JS 파일. 상세 화면 삭제 기능, enum 한글화(상세 전용 mapper·삼항식·JS mapping 포함 - T9C에서 중앙화), 목록 필터·페이지 상태 복원/URL 동기화/returnUrl(T9C 또는 후속), Dashboard 상세 링크(T9E)는 범위 밖.
- 위험: 관리자 CKEditor 본문 CSS가 `home.css`의 `.ckeditor-content`와 중복된다(Public 규칙 변경 시 admin.css 동기화 필요 - home.css는 변경하지 않으므로 admin.css 주석과 본 문서에 기록). 없는 ID는 기존 관례대로 `BOARD_NOT_FOUND`/`PROGRAM_NOT_FOUND` 404 JSON이다(관리자 HTML error page 없음). admin.css/template 동시 변경으로 브라우저 heuristic cache에 의한 "새 template + 구 CSS" 표시가 재발할 수 있다(검증은 cache 없는 context + 서빙 파일 hash 비교, 전역 cache 정책은 별도 후속 Task).
- 검증: `AdminBoardViewControllerTest`/`AdminProgramViewControllerTest`(비로그인 redirect, ADMIN 200·template·model, 비공개 조회, 공개 페이지 링크 조건, 없는 ID 404, `renderedContent` 외부 링크 처리, 목록/수정 링크, layout·sidebar active, Program 전용 필드), `visual-regression.spec.js` P14-T10 describe(목록 제목 → 상세 → 수정/목록, 공개/비공개 링크 조건, 375px overflow 0), 기존 admin-layout/admin-console-errors/visual-regression 무회귀, `./gradlew build`, Docker 실제 화면(1440/768/375), `git diff --check`.
- DoD: 목록 제목 클릭으로 상세 진입, 수정 폼 없이 내용 확인, 비공개 콘텐츠도 ADMIN 조회 가능, 상세 → 수정/목록 이동, 공개 페이지 보기는 `isPublic`일 때만, 본문은 `renderedContent` + `th:utext`만 사용(JS `innerHTML`/신규 raw HTML 경로 0), sidebar active 유지, 375px overflow 0, `/new`·`/{id}/edit` 무회귀, 기존 수정/삭제/공개 전환 무회귀, Public 무변경, T9C/T9D/T9E 미착수.
- 구현 결과(P14-T10, 최종): 위 설계대로 구현했다. **NEW 2개**(`admin/board/detail.html`, `admin/program/detail.html`), **CHANGE 11개**(Controller 2, list template 2, `admin.css`, Java test 2, `visual-regression.spec.js`, ARCHITECTURE/API/본 문서). 계획 대비 조정: home.css 11개 블록 중 `image-style-align-center`는 `.image`의 `margin: auto`와 결과가 같아 10개만 복사, 본문 링크 밑줄은 공통 `main a:not(.btn):not(.page-link)`보다 특이도를 높여(`a:not(.btn)`) 적용, 375px 표는 `th` 줄바꿈 금지 + 표 내부 가로 스크롤, 날짜는 기존 `#temporals.format` 관례에 시간을 더한 `yyyy.MM.dd HH:mm`. **Java**: `./gradlew build` JUnit 651/651 pass(신규 16). **Playwright**: `admin-layout.spec.js` 14/14, `admin-console-errors.spec.js` 12/12, `visual-regression.spec.js` 249/249(P14-T10 신규 4 포함) - 합계 275/275. **JS**: 355/355. **실제 화면**(Docker, cache 없는 context): 1440/768/375 overflow 0, 관리 정보 grid 3→2→1열, 이미지 25%/side/50%/inline 폭 정상, console/pageerror 0, 인증된 `/admin/boards/abc` 404, 없는 ID `BOARD_NOT_FOUND` 404, admin.css/template hash가 working tree·app JAR·nginx 서빙 모두 일치. `git diff --check` 통과, `docker-compose.local-test.yml` untracked 유지.

### P14-T11. Static Asset Cache Revalidation
- 의존성: P14-T10 완료 이후(P14-T9C/T9D/T9E 관리자 디자인 작업 이전에 선행)
- 산출물: `nginx/nginx.conf`(변경 - 정적 location 4곳에 `add_header Cache-Control "no-cache" always;` 각 1줄), `docs/ARCHITECTURE.md`("구현 전 확정 운영 계약 › Nginx 정적 리소스 공급"에 캐시 정책/운영 배포 전 결정 필요 항목 추가), 본 문서
- 작업 내용: Nginx가 host bind mount에서 직접 제공하는 `/css/`, `/js/`, `/images/`, `/vendor/` 응답에 `Cache-Control`/`Expires`가 없어 브라우저가 heuristic freshness(Last-Modified 기반 추정 유효기간) 동안 재검증 없이 이전 CSS/JS를 재사용하고, Spring이 `no-cache, no-store, max-age=0, must-revalidate`로 응답하는 HTML은 항상 최신이라 "최신 HTML + 이전 CSS/JS" 조합이 생기던 문제를 해결한다. **해결 대상 B**: HTTP/browser stale cache - 각 정적 location의 기존 `add_header X-Static-Served-By nginx always;` 바로 아래에 `add_header Cache-Control "no-cache" always;`를 추가한다(같은 레벨 add_header이므로 `X-Static-Served-By` 유지, server/http 레벨 add_header 없음, HTML용 `location /`에는 추가하지 않음). `no-cache`는 저장을 허용하되 사용 전 ETag/Last-Modified로 재검증하게 하는 정책이며 변경이 없으면 304로 응답한다(`no-store`/`max-age`/`immutable` 미사용). 공용 `nginx.conf`이므로 로컬 Docker와 기본 운영 Compose에 동일하게 적용한다. **이 정책은 versionless asset 구조의 안전한 기본 정책이며, versioned asset 도입 시 장기 캐시 + immutable로 재검토한다.** **범위 밖 A**: stale app image(template/Java 변경은 여전히 `docker compose ... up -d --build app` 필요)와, host checkout bind mount(static)와 app image(application/template)의 release lifecycle 불일치로 인한 atomic deployment/rollback 문제 - ARCHITECTURE.md에 운영 배포 전 결정 필요 항목으로만 기록한다.
- 변경 금지: `docker-compose.yml`/`docker-compose.local-test.yml`/`Dockerfile`/`application*.yml`/Java(`SecurityConfig` 포함)/Thymeleaf template/CSS/JS/테스트 파일/API.md/PRD.md/FEATURES.md. asset URL versioning(`?v=`, content/filename hash, Spring Resource Chain), `immutable`/장기 `max-age`, CDN, Nginx static image, release symlink/atomic deployment, gzip/`open_file_cache`/`proxy_cache`/`etag` 설정 변경, location 구조 리팩터링, 신규 테스트, P14-T9C/T9D/T9E 착수.
- 위험: 정적 리소스마다 conditional request(대부분 304) 왕복이 생긴다(프로젝트 규모상 허용). 향후 CDN 도입 시 origin 재검증이 늘어나므로 versioned asset과 함께 재검토한다. 새 정책은 이 헤더를 받은 응답부터 적용되므로 정책 도입 이전에 heuristic freshness 상태로 저장된 브라우저 캐시는 만료/새로고침으로 새 응답을 받기 전까지 남을 수 있다. bind mount된 `nginx.conf` 변경은 nginx가 자동 반영하지 않으므로 nginx 컨테이너 재생성이 필요하다. 이 Task는 B만 해결하며 A를 해결한 것으로 오해하지 않는다.
- 검증: `nginx -t`(nginx:1.27-alpine 일회성 컨테이너, `proxy_pass http://app:8080` upstream 해석을 위해 compose network 연결), `docker compose -f docker-compose.yml -f docker-compose.local-test.yml up -d --force-recreate --no-deps nginx`로 nginx만 재생성(app/db 무영향 확인), host/container `nginx.conf` hash 일치, `nginx -T`로 설정 4개 확인, `curl`로 `/css/home.css`·`/css/admin/admin.css`·`/js/home/nav-toggle.js`·`/js/admin/common-fetch.js`의 200(`Cache-Control: no-cache`/ETag/Last-Modified/`X-Static-Served-By`/Content-Type), `If-None-Match` 304, 존재하지 않는 정적 경로 404, HTML(`/`) 헤더 분리(static 헤더 미적용·Cache-Control 중복 없음), 기존 `./gradlew build`/Playwright/JS 회귀, `git diff --check`.
- DoD: 4개 정적 location이 200/304/404 모두 `Cache-Control: no-cache`를 단일 헤더로 반환, ETag/Last-Modified/`X-Static-Served-By`/Content-Type 유지, `If-None-Match` 304, HTML은 Spring의 기존 Cache-Control만 유지, `nginx -t` 통과, container 적용 설정 hash = 저장소 hash, 기존 테스트 무회귀, 변경 파일은 `nginx/nginx.conf`/`docs/ARCHITECTURE.md`/본 문서 3개뿐, `docker-compose.local-test.yml` untracked 유지.
