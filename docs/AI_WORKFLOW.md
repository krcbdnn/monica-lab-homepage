...
관련 문서 확인
(요구 변경 또는 불일치가 있을 때만 문서를 먼저 수정하고 리뷰/커밋)
...
본 프로젝트의 기본 구현 주체이며 다음 작업에서 사용한다.

- TASK.md의 P1-T1부터 마지막 Task까지 의존성/DoD 기반 순차 구현
- Entity/Repository/Service/Controller/View/Test 구현
- 프로젝트 전체 분석
- 리팩토링
- 구조 개선
- 중복 코드 제거
...
Task 실행 중 문서 충돌이나 누락된 결정이 발견되면 추측하거나 설계 문서를 임의 수정하지 않고 중단하여 보고한다.
...
최종 설계 확정 이후 일반 구현 Task에서는 문서를 자동 수정하지 않는다. 구현 중 문서 불일치를 발견하면 해당 Task를 중단하고 사용자에게 보고한다. 요구사항 변경이 승인된 경우에만 관련 Source of Truth 문서를 먼저 수정·검토·커밋한 뒤 구현을 재개한다. 문서 우선순위는 `CLAUDE.md`의 Source of Truth 규칙을 따른다.
