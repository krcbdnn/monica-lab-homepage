...
class AdminFileViewControllerTest extends AbstractIntegrationTest {
    ...
    @Test
    void listWithAuthenticationReturns200AndResolvesToFileListView() throws Exception {
        mockMvc.perform(get("/admin/files")
                        .with(user("admin").authorities(new SimpleGrantedAuthority("ROLE_ADMIN"))))
                .andExpect(status().isOk())
                .andExpect(view().name("admin/file/list"));
    }

    @Test
    void listRendersCommonAdminLayout() throws Exception {
        Document document = render(get("/admin/files"));
