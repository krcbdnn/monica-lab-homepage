...
class AdminBannerViewControllerTest extends AbstractIntegrationTest {
    ...
    @Test
    void listWithAuthenticationReturns200AndResolvesToBannerListView() throws Exception {
        mockMvc.perform(get("/admin/banners")
                        .with(user("admin").authorities(new SimpleGrantedAuthority("ROLE_ADMIN"))))
                .andExpect(status().isOk())
                .andExpect(view().name("admin/banner/list"));
    }
    ...
    @Test
    void newFormWithAuthenticationReturns200AndResolvesToBannerFormView() throws Exception {
        mockMvc.perform(get("/admin/banners/new")
                        .with(user("admin").authorities(new SimpleGrantedAuthority("ROLE_ADMIN"))))
                .andExpect(status().isOk())
                .andExpect(view().name("admin/banner/form"));
    }
    ...
    @Test
    void editFormWithAuthenticationReturns200AndResolvesToBannerFormViewEvenForNonExistentId() throws Exception {
        mockMvc.perform(get("/admin/banners/{id}/edit", 999_999L)
                        .with(user("admin").authorities(new SimpleGrantedAuthority("ROLE_ADMIN"))))
                .andExpect(status().isOk())
                .andExpect(view().name("admin/banner/form"));
    ...
    void listRendersCommonAdminLayout() throws Exception {
        Document document = render(get("/admin/banners"));
    ...
    void formRendersCommonAdminLayout() throws Exception {
        Document document = render(get("/admin/banners/new"));
