...
//   node --test src/test/js/admin/program-file-upload.test.js
'use strict';
...
const {
    ...
    mapUploadResponse,
    uploadFile
} = require('../../../main/resources/static/js/admin/program-file-upload.js');
...
test('templates/admin/program/form.html wires thumbnail/attachment uploads to the shared File API', () => {
    ...
    assert.match(html, /\/js\/admin\/program-file-upload\.js/);
    assert.match(html, /AdminProgramFileUpload\.uploadFile\(file, AdminProgramFileUpload\.THUMBNAIL_FILE_TYPE, AdminFetch\.adminFetch\)/);
    assert.match(html, /AdminProgramFileUpload\.uploadFile\(file, AdminProgramFileUpload\.ATTACHMENT_FILE_TYPE, AdminFetch\.adminFetch\)/);
    assert.match(html, /#thumbnail'\)\.value = url/);
    assert.match(html, /#attachment'\)\.value = url/);
});

test('templates/admin/program/form.html submits thumbnail/attachment fields to AdminProgramController via common fetch', () => {
    const templatePath = path.join(__dirname, '../../../main/resources/templates/admin/program/form.html');
    const html = fs.readFileSync(templatePath, 'utf8');

    assert.match(html, /AdminFetch\.adminFetch\(url, \{/);
    assert.match(html, /\/api\/admin\/programs/);
